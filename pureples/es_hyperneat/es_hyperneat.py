...
class AdaptiveESNetwork:
        ...
        for (x, y), idx in coords_to_id.items():
            for c in connections:
                if c.x2 == x and c.y2 == y:
                    ...
                    if idx in nodes:
                        ...
                        nodes[idx] = [[(coords_to_id[c.x1, c.y1], c.weight, c.a, c.b, c.c, c.d, c.n)], None]
            ...
            for c in mod_connections:
                if c.x2 == x and c.y2 == y:
                    ...
                    if idx in nodes:
                        if nodes[idx][1]:
                            if coords_to_id.get((c.x1, c.y1), False):
                                initial = nodes[idx][1]
                                initial.append((coords_to_id[c.x1, c.y1], c.weight))
                                nodes[idx][1] = initial
                        else:
                            if coords_to_id.get((c.x1, c.y1), False):
                                nodes[idx][1] = [(coords_to_id[c.x1, c.y1], c.weight)]
                    else:
                        if coords_to_id.get((c.x1, c.y1), False):
                            nodes[idx] = [None, [(coords_to_id[c.x1, c.y1], c.weight)]]

        # Combine the indices with the connections/links;
        # forming node_evals used by the RecurrentNetwork.
        for idx, links in nodes.items():
            node_evals.append((idx, self.activation, sum, 0.0, 1.0, links[0], links[1]))
    ...
    def division_initialization(self, coord, outgoing, cppn_output):
        """
        Initialize the quadtree by dividing it in appropriate quads.
        """
        root = QuadPoint(0.0, 0.0, 1.0, 1)
        ...
        while q:
            ...
            for c in p.cs:
                c.w, c.a, c.b, c.c, c.d, c.n = query_cppn_adaptive_es(coord, (c.x, c.y), outgoing,
                                 self.cppn, cppn_output, self.max_weight)
    ...
    def pruning_extraction(self, coord, p, outgoing, cppn_output):
        """
        Determines which connections to express - high variance = more connetions.
        """
        for c in p.cs:
            ...
            if self.variance(c) > self.variance_threshold:
                self.pruning_extraction(coord, c, outgoing, cppn_output)
            else:
                d_left = abs(c.w - query_cppn_adaptive_es(coord, (c.x - p.width,
                                                      c.y), outgoing, self.cppn, cppn_output, self.max_weight)[cppn_output])
                d_right = abs(c.w - query_cppn_adaptive_es(coord, (c.x + p.width,
                                                       c.y), outgoing, self.cppn, cppn_output, self.max_weight)[cppn_output])
                d_top = abs(c.w - query_cppn_adaptive_es(coord, (c.x, c.y - p.width),
                                                        outgoing, self.cppn, cppn_output, self.max_weight)[cppn_output])
                d_bottom = abs(c.w - query_cppn_adaptive_es(coord, (c.x, c.y +
                                                        p.width), outgoing, self.cppn, cppn_output, self.max_weight)[cppn_output])

                con = None
                if max(min(d_top, d_bottom), min(d_left, d_right)) > self.band_threshold:
                    if outgoing:
                        con = Connection(coord[0], coord[1], c.x, c.y, c.w, c.a, c.b, c.c, c.d, c.n)
                    else:
                        con = Connection(c.x, c.y, coord[0], coord[1], c.w, c.a, c.b, c.c, c.d, c.n)
                if con is not None:
                    # Nodes will only connect upwards.
                    # If connections to same layer is wanted, change to con.y1 <= con.y2.
                    if not c.w == 0.0 and con.y1 < con.y2 and not (con.x1 == con.x2 and con.y1 == con.y2):
    ...
    def adaptive_es_hyperneat(self, cppn_output):
        """
        Explores the hidden nodes and their connections.
        """
        inputs = self.substrate.input_coordinates
        ...
        for x, y in inputs:  # Explore from inputs.
            root = self.division_initialization((x, y), True, cppn_output)
            self.pruning_extraction((x, y), root, True, cppn_output)
            connections1 = connections1.union(self.connections)
            for c in connections1:
                hidden_nodes.add((c.x2, c.y2))
            self.connections = set()
        ...
        for x, y in outputs:  # Explore to outputs.
            root = self.division_initialization((x, y), False, cppn_output)
            self.pruning_extraction((x, y), root, False, cppn_output)
            connections3 = connections3.union(self.connections)
            self.connections = set()
...
class Connection:
    ...
    def __init__(self, x1, y1, x2, y2, weight, a = None, b = None, c = None, d = None, n = None):
        self.x1 = x1
        self.y1 = y1
        self.x2 = x2
        self.y2 = y2
    ...
    def __hash__(self):
        return hash((self.x1, self.y1, self.x2, self.y2, self.weight, self.a, self.b, self.c, self.d, self.n))
